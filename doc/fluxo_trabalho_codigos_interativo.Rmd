...
Esse gráfico pode melhorar. Compartilhar o eixo x? Abreviar o nome das espécies, adicionar título ao eixo x...
...
```{r}
n_obs_uc_sp_ano <- contar_n_obs_uc_sp_ano()
n_obs_uc_sp_ano
```
...
```{r}
gerar_tabdin_n_obs_uc_sp_ano()
```

#### **Quantas observações para cada espécies por UC, por estação e por ano?**

```{r}
n_obs_uc_sp_estacao_ano <- contar_n_obs_uc_sp_estacao_ano()

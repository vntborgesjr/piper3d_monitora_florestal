...
```{r}
dados <- filtrar_dados(validacao_obs = "especie")
dados |> 
  dplyr::group_by(nome_sp) |> 
  dplyr::count(nome_uc)
```
...
dados |> 
  dplyr::group_by(nomes_sp) |> 
  count(data_amostragem, nome_ea, nome_uc)
```
...
dados |> 
  dplyr::group_by(nomes_sp) |> 
  count(data_amostragem, nome_ea, nome_uc, ano)
```
...
dados |> 
  dplyr::group_by(nomes_sp) |> 
  count(data_amostragem, nome_ea, nome_uc, estacao)
```
...
relação a distância, é possível que estejam influenciando na
...
frequência em distâncias maiores no seu horário de maior atividade.
...
dados_filtrados |> 
  plotar_distancia_covariavel(
    ...
    y = tamanho_grupo,
    xlab = "Distância (m)",
    ylab = "Tamanho do grupo"
  )
```

# **PARTE IV -- Ajustando os modelos**
...
##### HN + Tamanho do grupo + tempo decorrido desde o início das observações (HN + S + T)

É muito importante eliminar as observações ausentes (`NA`s) antes de ajustar os modelos com covariáveis caso contrário, o modelo não será ajustado.

```{r}
# ajustando a função de detecção half-normal, com tamanho do grupo como covariável, para um truncamento de 10% dos dados
cutias_cov_distance_hn_size_time <- cutias_cov_distance |> 
  tidyr::drop_na(cense_time) |> # essa linha de comando elimina os NAs
  ajustar_modelos_Distance(
    funcao_chave = "hn",
    truncamento = "10%",
...
###### HZ + Tamanho do grupo + tempo decorrido desde o início das observações (HZ + S + T)
...
# eliminar NAs dos dados
cutias_cov_distance <- cutias_cov_distance |> 
  tidyr::drop_na(cense_time)

resultados_bootstrap <- gerar_intervalos_confianca_bootstrap(
  dados = cutias_cov_distance,
  funcao_chave = "hr", 
  termos_ajuste = NULL,
  truncamento = "10%",
  formula = ~ size + cense_time, 
  funcao_sumario = "densidade", 
  nboot = 300,
  nucleos = 7
)

resultados_bootstrap$intervalo_confianca
resultados_bootstrap$histograma
```

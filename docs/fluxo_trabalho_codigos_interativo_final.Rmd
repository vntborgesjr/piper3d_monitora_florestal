...
###### Quantas observações foram validadas por nível taxonômico?
...
###### Quantas Unidades de Conservação foram amostradas?
...
###### Quantas observações por Unidade de Conservação?
...
independentes. Segundo BUCKLAND et al. (2015), o número mínimo de
réplicas independentes para os transectos deve ser de 10-20, o que deve
...
caso de muitas UCs, chegando ao máximo de 9 estações amostrais por UC. O
baixo número de réplicas espaciais idependentes não chega a ser um
problema na estimativa de densidade da Área Coberta pela amostragem, mas
limita uma boa estimativa de densidade total para a Unidade de
Conservação.
...
repetidas e nã independentes levam a uma inflagem no tamanho da área
coberta. Apenas para ilustar com um exemplo, para os dados da cutia
*Dasyprocta croconota* da Resex Tapajós-Arapinuns, a Estação Amostral
...
fluxo de trabalho até a presente versão e pretendemos testar os códigos
para esta abordagem utilizando o pacote unmarked até o final da vigência
do presente projeto.
...
Tapajos-Arapiuns](%60r%20here::here(%22output/Distribuicao_freq_Dasyprocta_ResexTapajos.png%22)%60){width="75%"}
...
fator seria um possível arredondamento nas distâncias próximas a zero, o
que pode gerar um "amontoamento" nos dados (*heaping*). Recomenda-se que
...
distância. Caso o viés ocorra, será uma observada uma tendência de
...
Espécies que formam grupos devem ter a covariável 'size'. As estratégias
de estratificação podem ser substituídas por covariáveis também
(estratos espaciais/ ano). Como covariável temporal, pode se pensar em
usar, além do ano, a estação do ano (season), o horário do dia (para
animais que variam a atividade). O horário do dia pode ser convertido em
tempo após nascer do sol (como no exemplo). Mas para isso é necessário
criar essa variável no dataset. E não é trivial porque precisa saber o
horário de nascer do sol em cada dia/local para calcular.

...
Para que o arquivo
`fluxo_de_trabalho_codigos_interativo_final.Rmd` funcione de maneira
...
#REVISAR ESSE trecho no final Especies com maior numero de
...
Estratificação nos dados com as espécies com maior número de
observações. Por isso, selecionamos como modelo a espécie com o maior
número de observações para uma mesma UC e a terceira com o maior número
de total de observações.
...
observações detectadas nas maiores distãncias. Vamos selecionar o valor
de corte pelo modelo com melhor ajuste. Para automatizar a avaliação da
...
observações mais distantes. Geramos uma tabela comparando os modelos e
...
```{r}
bondade_ajuste_fluxo1 <- testar_bondade_ajuste(fluxo1_modelos_selecionados, 
  plot = TRUE,
  chisq = TRUE, 
...
```{r}
bondade_ajuste_fluxo2 <- testar_bondade_ajuste(fluxo2_modelos_selecionados, 
  plot = TRUE,
  chisq = TRUE, 
  intervalos_distancia  =  NULL
)
...
fluxo3.1_selecao_funcao_deteccao_termo_ajuste <- selecionar_funcao_deteccao_termo_ajuste(
  dados = fluxo3.1_lista_modelos_ajustados, 
...
fluxo3.2_selecao_funcao_deteccao_termo_ajuste <- selecionar_funcao_deteccao_termo_ajuste(
  dados = fluxo3.2_lista_modelos_ajustados, 
  distancia_categorizada = TRUE
)
...
fluxo3.3_selecao_funcao_deteccao_termo_ajuste <- selecionar_funcao_deteccao_termo_ajuste(
  dados = fluxo3.3_lista_modelos_ajustados, 
  distancia_categorizada = TRUE
)
...
# atribuir o nome dos modelos aos itens da lista
names(fluxo3.1_modelos_selecionados) <- fluxo3.1_selecao_funcao_deteccao_termo_ajuste$Modelo
...
names(fluxo3.1_modelos_selecionados) <- fluxo3.2_selecao_funcao_deteccao_termo_ajuste$Modelo
...
names(fluxo3.3_modelos_selecionados) <- fluxo3.1_selecao_funcao_deteccao_termo_ajuste$Modelo
...
Aqui vamos trabalhar com os dados filtrados, sem repetições. O número de observações total foi de 71.
...
observar que nesse caso as covariávies alteraram pouco o ajuste dos modelos.
...
```{r}
cutias_tap <- filtrar_dados(
  nome_ucs = "resex_tapajos_arapiuns",
  nome_sps = "dasyprocta_croconota",
  validacao_obs = "especie"
)

cutias_tap_distance <- transformar_dados_formato_Distance(
  cutias_tap,
  amostras_repetidas = TRUE)
...
```{r}
cutias_par <- filtrar_dados(
  nome_ucs = "parna_da_serra_do_pardo",
  nome_sps = "dasyprocta_croconota",
  validacao_obs = "especie"
)

cutias_par_distance <- transformar_dados_formato_Distance(
  cutias_par,
  amostras_repetidas = TRUE)
...
```{r}
cutias_ter <- filtrar_dados(
  nome_ucs = "esec_da_terra_do_meio",
  nome_sps = "dasyprocta_croconota",
  validacao_obs = "especie"
)

cutias_ter_distance <- transformar_dados_formato_Distance(
  cutias_ter,
  amostras_repetidas = TRUE)
...
```{r}
cutias_anf <- filtrar_dados(
  nome_ucs = "resex_riozinho_do_anfrisio",
  nome_sps = "dasyprocta_croconota",
  validacao_obs = "especie"
)

cutias_anf_distance <- transformar_dados_formato_Distance(
  cutias_anf,
  amostras_repetidas = TRUE)
...
Para ajustar os modelos nos dados globais e estratificados, mantivemos a mesma distância de truncamento para os dados das cutias na Resex Tapajós_Arapiuns, de 10%, uma vez que a distrbuição dos dados é semelhante tanto nos dados globais como nas demais UCs.
...
###### ARRUMAR Tabela com o resumo comparativo dos modelos

```{r, eval=FALSE}
#AQUI PRECISA AJUSTAR PARA QUE SEJA POSSIVEL GERAR UMA TABELA COM VALORES BRUTOS DE AIC QUE NÃO COMPARE OS AICS, PORQUE OS Ns são diferentes.

modelo_cutias_estrat <- selecionar_funcao_deteccao_termo_ajuste(
  cutias_distance_hn$Cosseno,
  cutias_tap_distance_hn$Cosseno,
  cutias_par_distance_hn$Cosseno,
  cutias_ter_distance_hn$Cosseno,
  cutias_anf_distance_hn$Cosseno,
  delta = TRUE
)


melhor_modelo_cutias_estrat

```
###### ARRUMAR Gráficos de ajuste das funções de deteção às probabilidades de deteção

```{r, eval=FALSE}
## PRECISA DO OBJETO CRIADO NA FUNÇÃO ANTERIOR
...
modelos_cutias_estrat <- gerar_lista_modelos_selecionados(
  ...
  cutias_distance_unif$`Polinomial simples`,
  cutias_distance_hn$`Sem termo`,
  nome_modelos_selecionados = melhor_modelo_cutias_estrat
)

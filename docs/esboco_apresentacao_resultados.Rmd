...
```{r}
bondade_ajuste <- fluxo1_modelos_selecionados |> 
  purrr::map(
...
bondade_ajuste |> 
  ...
  list_rbind() |> 
  mutate(Modelo = names(fluxo1_modelos_selecionados)) |> 
...
fluxo1_area_esforco_hr <- cutia_tap_arap_hr$`Sem termo` |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
gerar_caracteristicas_area_estudo_taxa_encontro(cutia_tap_arap_hn$Cosseno)
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
gerar_caracteristicas_area_estudo_taxa_encontro(cutia_tap_arap_unif$Cosseno)
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
gerar_caracteristicas_area_estudo_taxa_encontro(cutia_tap_arap_unif$`Polinomial simples`)
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
gerar_caracteristicas_area_estudo_taxa_encontro(cutia_tap_arap_hn$`Sem termo`)
```
...
###### Caracteristicas da área e esforço

```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
fluxo1_area_esforco_hr_sem_termo <- cutia_tap_arap_hr$`Sem termo` |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

fluxo1_area_esforco_hr_cosseno <- cutia_tap_arap_hn$Cosseno |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

fluxo1_area_esforco_unif_cosseno <- cutia_tap_arap_unif$Cosseno |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

fluxo1_area_esforco_unif_polinomial <- cutia_tap_arap_unif$`Polinomial simples` |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

fluxo1_area_esforco_hn_sem_termo <- cutia_tap_arap_hn$`Sem termo` |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

# é importante que a ordem de entrada dos objetos respeitem a ordem da tabela de selecao de modelos
fluxo1_area_esforco_hr_sem_termo |> 
  rbind(fluxo1_area_esforco_hr_cosseno) |> 
  rbind(fluxo1_area_esforco_unif_cosseno) |> 
  rbind(fluxo1_area_esforco_unif_polinomial) |> 
  rbind(fluxo1_area_esforco_hn_sem_termo) |> 
  mutate(Model = fluxo1_selecao_funcao_deteccao_termo_ajsute$Model) |> # pode ser um argumento da função
  relocate(Model, .before = Region)
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
area_esforco_hr <- cutia_tap_arap_quase_sem_repeticao_hr$`Sem termo` |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
gerar_caracteristicas_area_estudo_taxa_encontro(cutia_tap_arap_quase_sem_repeticao_hn$Cosseno)
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
gerar_caracteristicas_area_estudo_taxa_encontro(cutia_tap_arap_quase_sem_repeticao_unif$Cosseno)
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
gerar_caracteristicas_area_estudo_taxa_encontro(cutia_tap_arap_quase_sem_repeticao_unif$`Polinomial simples`)
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
gerar_caracteristicas_area_estudo_taxa_encontro(cutia_tap_arap_quase_sem_repeticao_hn$`Sem termo`)
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
area_esforco_hr_sem_termo <- cutia_tap_arap_quase_sem_repeticao_hr$`Sem termo` |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

area_esforco_hr_cosseno <- cutia_tap_arap_quase_sem_repeticao_hn$Cosseno |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

area_esforco_unif_cosseno <- cutia_tap_arap_quase_sem_repeticao_unif$Cosseno |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

area_esforco_unif_polinomial <- cutia_tap_arap_quase_sem_repeticao_unif$`Polinomial simples` |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

area_esforco_hn_sem_termo <- cutia_tap_arap_quase_sem_repeticao_hn$`Sem termo` |> 
  gerar_caracteristicas_area_estudo_taxa_encontro()

# é importante que a ordem de entrada dos objetos respeitem a ordem da tabela de selecao de modelos
area_esforco_hr_sem_termo |> 
  rbind(area_esforco_hr_cosseno) |> 
  rbind(area_esforco_unif_cosseno) |> 
  rbind(area_esforco_unif_polinomial) |> 
  rbind(area_esforco_hn_sem_termo) |> 
  mutate(Model = selecao_funcao_deteccao_termo_ajsute$Model) |> # pode ser um argumento da função
  relocate(Model, .before = Region)
```

...
```{r}
dasy_croc <- transformar_para_distanceR_covariaveis_sem_repeticao() |> 
  filter(sp_name == "Dasyprocta croconota") |> 
  drop_na(distance)
...
dados_para_filtrar_por_data_quase_sem_repeticao <- teste1 |> 
  right_join(teste2, join_by(Sample.Label, year, season, n == n_max)) |> 
  arrange(sampling_day) |>
  filter(!(sampling_day %in% as.Date(c("2015-01-03",
                                       "2015-05-05",
                                       "2015-10-09",
                                       "2015-10-23",
                                       "2016-02-21",
...
dasy_croc_quase_sem_repeticao <- dasy_croc |> 
  filter(sampling_day %in% filtro_datas_quase_sem_repeticao,
         Region.Label == "Parna da Serra do Pardo")

dasy_croc_quase_sem_repeticao |> 
  plotar_distribuicao_distancia_interativo()
```
...
```{r}
dasy_croc_quase_sem_repeticao_hn <- dasy_croc_quase_sem_repeticao |> 
...
```{r}
dasy_croc_quase_sem_repeticao_hr <- dasy_croc_quase_sem_repeticao |> 
...
```{r}
dasy_croc_quase_sem_repeticao_hn |> 
  purrr::map(\(.x) plot(.x)) 
```

```{r}
dasy_croc_quase_sem_repeticao_hr |> 
  purrr::map(\(.x) plot(.x)) 
```
...
```{r}
summarize_ds_models(
  dasy_croc_quase_sem_repeticao_hn$`Sem termo`,
  dasy_croc_quase_sem_repeticao_hn$Cosseno,
  dasy_croc_quase_sem_repeticao_hn$`Hermite polinomial`,
  dasy_croc_quase_sem_repeticao_hr$`Sem termo`,
  dasy_croc_quase_sem_repeticao_hr$Cosseno,
  dasy_croc_quase_sem_repeticao_hr$`Polinomial simples`
)
```
...
```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
dasy_croc_quase_sem_repeticao_hn$`Sem termo`$dht$individuals$summary[1:9]
```

```{r}
# área de estudo, tamanho da área de estudo, trilhas ou estações amostrais, esforço total em cada trilha, abundância estimada em cada estação amostral, número de detecções em cada estação amostral, área total amostrada
dasy_croc_quase_sem_repeticao_hn$`Sem termo`$dht$individuals$Nhat.by.sample[1:8]
```

```{r}
# total, densidade estimada, erro padrão da densidade destimada, coeficiente de variação da densidade destimada, intervalo de confiança inferior e superior do coeficiente de variação, gruas de liberdade
dasy_croc_quase_sem_repeticao_hn$`Sem termo`$dht$individuals$D

```

```{r}
# área de estudo, tamanho da área de estudo, area coberta pelo esforço amostral, esforço amostral em metros, número de detecções, número de transectos (ea), taxa de encontro, coeficiente de variação da taxa de encontro  
dasy_croc_quase_sem_repeticao_hr$`Sem termo`$dht$individuals$summary[1:9]
```

```{r}
# área de estudo, tamanho da área de estudo, trilhas ou estações amostrais, esforço total em cada trilha, abundância estimada em cada estação amostral, número de detecções em cada estação amostral, área total amostrada
dasy_croc_quase_sem_repeticao_hr$`Sem termo`$dht$individuals$Nhat.by.sample[1:8]
```

```{r}
# total, densidade estimada, erro padrão da densidade destimada, coeficiente de variação da densidade destimada, intervalo de confiança inferior e superior do coeficiente de variação, gruas de liberdade
dasy_croc_quase_sem_repeticao_hr$`Sem termo`$dht$individuals$D
